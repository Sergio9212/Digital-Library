...
const Register: React.FC<RegisterProps> = ({ onToggleMode }) => {
  ...
  const formDataRef = useRef<RegisterRequest>(formData);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Solo limpiar el error cuando el registro sea exitoso
      setError('');
    } catch (error: any) {
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: '8px' }}>
            ...
            >
              ¿Ya tienes cuenta? <span style={{ fontWeight: '600' }}>Inicia sesión aquí</span>
            </button>
          </div>
        </form>
      </div>
